...
export const CSVUploader = ({
  ...
  const stripSpaces = (s: string): string =>
    s.replace(
      /[\s\u00A0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]+/g,
      "",
    );
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          ...
          cleaned.toLowerCase().includes("weight")
        ) {
          continue;
        }
      ...
      if (!targetHiveId) {
        ...
        const match = hives?.find(
          (h) => h.hive_code === hiveCode || h.name === hiveCode,
        );
        if (!match) {
          throw new Error(
            `No hive found matching code "${hiveCode}". Please create the hive first or select one.`,
          );
        }
      ...
      console.log(
        `[CSV Debug] dataStartIndex=${dataStartIndex}, total lines=${lines.length}, hiveCode=${hiveCode}`,
      );
      ...
      for (let i = dataStartIndex; i < lines.length; i++) {
        ...
        if (i === dataStartIndex) {
          ...
          console.log(
            `[CSV Debug] First data line cols (${cols.length}):`,
            cols,
          );
        }
        if (cols.length < 7) {
          if (i === dataStartIndex)
            console.log(
              `[CSV Debug] Skipped line ${i}: cols.length=${cols.length} < 7`,
            );
          continue;
        }
        ...
        if (!ts) {
          if (i === dataStartIndex)
            console.log(
              `[CSV Debug] Skipped line ${i}: parseTimestamp failed for "${cols[0]}"`,
            );
          continue;
        }
        ...
        if (timestamps.has(tsKey)) {
          throw new Error(
            `Duplicate timestamp found at line ${i + 1}: ${cols[0]}`,
          );
        }
      ...
      if (existing && existing.length > 0) {
        throw new Error(
          `${existing.length} duplicate timestamp(s) already exist in this hive. Re-uploads of corrected data are not allowed.`,
        );
      }
